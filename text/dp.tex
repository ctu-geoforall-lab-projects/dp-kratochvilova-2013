...
\fancyfoot[R]{\thepage}
\fancyfoot[L]{\textit{Anna Kratochvílová, CTU in Prague, 2012}}
...
\newcommand{\dash}{\mbox{\,---\,}}
 ...
 \hypersetup{
    ...
    pdfauthor={Anna Kratochv\'{\i}lov\'{a}},
...
\newenvironment{mtabstract}[1][Abstract]{%
\chapter*{\centering #1}%
\thispagestyle{empty}
\newcommand{\keyvalue}[2]{\noindent\textbf{##1: }##2}
\newcommand{\keywordsskip}{\bigskip
\bigskip
\bigskip
\bigskip
}
...
which resulted in TSQL2~\cite{snodgrass1995}.
...
\begin{figure}[h!]
  \centering
  \includegraphics{./images/cyclical_time.png}
...
According to the producer, it can be used for environmental, coastal, marine, health, crime, sales and marketing,
...
\noindent Also, GUI (automatically generated dialogs) for temporal modules
was improved as part of this work.
...
ffmpeg -r 10 -i input_%02d.png -sameq -vcodec mpeg4 output.avi
    \end{lstlisting}
    \end{footnotesize}
      where parameter \emph{r} specifies frame rate (frames per second),
      \emph{i} input files (requires sequential numbering of files which is ensured by the \at) and
...
that the duration of each key frame (represented by a map) is given by the given time stamp information.
...
Images for 3D view animation must be first rendered (to a file) by the module \module{m.nviz.image}~\cite{grassUserMan}.
This rendering is more time-consuming than loading raster maps for the 2D view,
...
Moreover, we can animate points or lines, too.
...
Also, an easy-to-follow video tutorial is available on GRASS-Wiki page~\cite{mapswipeWiki}.
...
\end{itemize}
In the future, such tool could be integrated into any wxGUI application processing temporal data
and then, it could be used as a control widget.


\section{GUI for temporal modules}
Although the \tf is fully working from command line, until now it was not introduced in the GRASS wxGUI.
This means that automatically generated GUI dialogs (refer to \ref{sec:grasswxgui})
for all temporal modules worked only partially because the widget for the selection of GRASS elements
(like raster, 3D raster, vector, region, etc.) was not able to display the new temporal elements
(space time datasets). As a~consequence, user had to type the name of the dataset manually
...
% Moreover, in the future it could be integrated  into another tool
